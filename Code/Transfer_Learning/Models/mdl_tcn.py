...
def create_model(hp):
    ...
    for i in range(hp.Int('num_layers', min_value=1, max_value=3, default=1)):
        nb_filters = hp.Int('nb_filters_' + str(i), min_value=10, max_value=70, default=64)
        kernel_size = hp.Int('kernel_size_' + str(i), min_value=2, max_value=10, default=2)
        nb_stacks = hp.Int('nb_stacks_' + str(i), min_value=1, max_value=6, default=1)
        dilations = hp.Int('dilations_' + str(i), min_value=3, max_value=9, default=6)  # [1, 2, 4, 8, 16, 32]
        padding = hp.Choice('padding_' + str(i), ['causal', 'same'], default='causal')
        use_skip_connections = hp.Boolean('use_skip_connections_' + str(i), default=False)
        dropout_rate = hp.Float('dropout_rate_' + str(i), min_value=0.0, max_value=0.9, default=0.05)
        model.add(TCN(input_shape=[25, 56],
...
def tune_model(hp):
    ...
    model = models.load_model(model_path, custom_objects={'TCN': TCN})
...
def fine_tune(mdl_path='best_model.h5', optimizer=tf.optimizers.Adam(learning_rate=1e-4)):
    ...
    """
    # fine tune
    model = models.load_model(mdl_path, custom_objects={'TCN': TCN})  # custom_objects is needed for loading a TCN model
